...
# %%
# output arrays
temp_out = np.ones((276, 3, 841)) * np.nan
faer_out = np.ones((276, 3, 841)) * np.nan
fnat_out = np.ones((276, 3, 841)) * np.nan
fant_out = np.ones((276, 3, 841)) * np.nan
...
scenarios = ['counterfactual', 'hunga', 'solar']
...
for scenario in scenarios:
    fill(f.forcing, df_solar[scenario].values[:, None] * df_configs['forcing_scale[Solar]'].values[None, :], specie='Solar', scenario=scenario)
    fill(f.forcing, df_volcanic[scenario].values[:, None] * df_configs['forcing_scale[Volcanic]'].values[None, :], specie='Volcanic', scenario=scenario)
...
temp_out[:, :, :] = f.temperature[:, :, :, 0]# - np.average(f.temperature[100:152, 0, :, 0], weights=weights, axis=0)
faer_out[:, :, :] = f.forcing[:, :, :, 54] + f.forcing[:, :, :, 55]
fnat_out[:, :, :] = f.forcing[:, :, :, 52] + f.forcing[:, :, :, 53]
fant_out[:, :, :] = f.forcing_sum[:, :, :] - fnat_out[:, :, :]

# %%
pl.plot(np.median(temp_out[:, :, :] - np.average(f.temperature[100:152, :, :, 0], weights=weights, axis=0), axis=2));

# %%
np.median(temp_out[250:, :, :] - np.average(f.temperature[100:152, :, :, 0], weights=weights, axis=0), axis=2)
